...
func EpayNotify(c *gin.Context) {
	writeResult := func(result string) {
		if _, writeErr := c.Writer.Write([]byte(result)); writeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 path=%q client_ip=%s result=%s error=%q", c.Request.RequestURI, c.ClientIP(), result, writeErr.Error()))
		}
	}
	...
	result, completeErr := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type)
	if completeErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 原子结算失败 trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), completeErr.Error()))
		writeResult("fail")
		return
	}
	if result.NewlyCredited {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f", result.TopUp.TradeNo, result.TopUp.UserId, c.ClientIP(), result.QuotaToAdd, result.TopUp.Money))
		model.RecordTopupLog(result.TopUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(result.QuotaToAdd), result.TopUp.Money), c.ClientIP(), result.TopUp.PaymentMethod, model.PaymentProviderEpay)
	}
	writeResult("success")
}
