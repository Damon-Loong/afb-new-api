...
// defaultTieredPreConsumeMaxTokens is used when the request omits max_tokens.
...
func ContainPriceOrRatio(modelName string) bool {
	return HasModelBillingConfig(modelName)
}

// HasModelBillingConfig checks all supported billing modes for a model.
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo types.GroupRatioInfo) (types.PriceData, error) {
	...
	logger.LogDebug(c, fmt.Sprintf("model_price_helper_tiered result: model=%s preConsume=%d quotaBeforeGroup=%.2f groupRatio=%.2f tier=%s", info.OriginModelName, preConsumedQuota, quotaBeforeGroup, groupRatioInfo.GroupRatio, trace.MatchedTier))
	info.PriceData = priceData
	return priceData, nil
}
