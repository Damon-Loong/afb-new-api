...
import DynamicPricingLogDetail from '../../components/table/usage-logs/components/DynamicPricingLogDetail';
...
export const useLogsData = () => {
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (logs[i].type === 2 && other?.billing_mode !== 'tiered_expr') {
        expandDataLocal.push({
          key: t('日志详情'),
          value: other?.claude
            ? renderClaudeLogContent(
      ...
      if (logs[i].type === 2) {
        ...
        if (other?.billing_mode === 'tiered_expr' && other?.expr_b64) {
          expandDataLocal.push({
            key: t('计费过程'),
            value: <DynamicPricingLogDetail other={other} t={t} />,
          });
        }
